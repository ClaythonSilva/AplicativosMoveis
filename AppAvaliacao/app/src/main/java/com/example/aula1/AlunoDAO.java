...
import com.example.aula1.Aluno;
import com.example.aula1.Conexao;
...
public class AlunoDAO {
    ...
    public List<Aluno> buscarPorNomeInicial(String nomeInicial){
        List<Aluno> alunosFiltrados = new ArrayList<>();
        Cursor cursor = banco.query("aluno", new String[]{"id", "nome", "cpf", "telefone", "fotoBytes"},
                "nome LIKE ?", new String[]{nomeInicial + "%"}, null, null, null);
        while(cursor.moveToNext()){
            Aluno a = new Aluno();
            a.setId(cursor.getInt(0));
            a.setNome(cursor.getString(1));
